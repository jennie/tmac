  ...
  models: [
      ...
      transform: data => {
        if (data.options) {
          ...
          options.forEach(function(option, index) {
            option.npRate = numeral(
              Math.round(option.rate * serviceNpRate)
            ).format("$0,0[.]00");
        ...
        if (data.rates) {
          ...
          rates.forEach(function(rate, index) {
            if (rate.period.extraHours) {
              ...
              rate.npRate = numeral(
                Math.round(rate.rate * npRate + staffingCost)
              ).format("$0,0[.]00");
              rate.rate = numeral(Math.round(rate.rate + staffingCost)).format(
                "$0,0[.]00"
              );
            } else {
              rate.npRate = numeral(Math.round(rate.rate * npRate)).format(
                "$0,0[.]00"
              );
    ...
    },
    {
      ...
      transform: data => {
        ...
        if (
          dateFns.isBefore(dateFns.parseISO(data.startDateTime), new Date())
        ) {
          data.past = true;
        } else {
          data.past = false;
        ...
        var zonedStart = dateFnsTz.utcToZonedTime(
          data.startDateTimeParsed,
          "America/New_York"
        );
        var zonedEnd = dateFnsTz.utcToZonedTime(
          data.endDateTimeParsed,
          "America/New_York"
        );
        ...
        if (data.endDateTime && data.startDateTime) {
          ...
          if (dateFns.isSameDay(zonedStart, zonedEnd) == true) {
            ...
            data.humanTime = `${data.startDateTime.toLocaleString({
              month: "long",
              day: "numeric"
            })}–${data.endDateTime.toLocaleString({ day: "numeric" })}`;
          } else {
            data.humanTime = `${data.startDateTime.toLocaleString({
              month: "long",
              day: "numeric"
            })}–${data.endDateTime.toLocaleString({
              month: "long",
              day: "numeric"
            })}`;
          }
    ...
    },
    {
      ...
      template: {
        ...
        output: program => {
          return `program/${program.slug}.html`;
        }
      },
      transform: data => {
        var endOfDayEndDate = dateFns.endOfDay(dateFns.parseISO(data.endDate));
        var startOfDayStartDate = dateFns.startOfDay(
          dateFns.parseISO(data.startDate)
        );
        ...
        if (
          dateFns.isWithinInterval(new Date(), {
        ...
        if (dateFns.isSameDay(zonedStart, zonedEnd) == true) {
          ...
          data.humanTime = `${data.startDate.toFormat(
            "LLLL d"
          )}–${data.endDate.toFormat("d")}`;
          // console.log(data.title, data.humanTime, zonedEnd)
          return data;
        } else {
          data.humanTime = `${data.startDate.toLocaleString({
            month: "long",
            day: "numeric"
...
module.exports = {
  ...
  reshape: htmlStandards({
    ...
    markdownPlugins: [
      [markdownItTocAndAnchor, { tocFirstLevel: 3, anchorLink: false }],
      markdownItAttrs,
      markdownItContainer
    ],
    retext: { quotes: false }
